...
# ============================================================
# Domain
#   Axes (matches original code & VTK):
#     x : laser scanning direction
#     y : VERTICAL (depth) -- y=0 is cold base, y=Ly is the free top surface
#         (where the laser enters; melt pool sits here in the original code)
#     z : transverse (track-width direction)
#
#   Single-track deposit grows in +y direction on top of the substrate:
#     - cells with j in [0, ny_substrate)  : SUBSTRATE   (born from t=0)
#     - cells with j in [ny_substrate, ny) : DEPOSIT     (born only when laser arrives)
# ============================================================
dx = dy = dz = 0.05
nx, nz = 250, 50

# >>> NEW: substrate / deposit decomposition (along y, the vertical axis)
ny_substrate = 50              # original substrate thickness in y
ny_deposit   = 5              # extra layers added above the substrate for the deposit
ny           = ny_substrate + ny_deposit   # total y-extent used by the solver
...
# ============================================================
# >>> NEW: single-track / element-birth parameters
# ============================================================
# Track is along the laser path (x).  Width is in z (transverse).  Height is in y.
z_track_center      = nz * dz / 2.0                   # same as laser z_center: middle of z
track_half_width_z  = 20.0 * dz                        # ~5 cells of half-width in z
y_track_min         = ny_substrate * dy               # bottom of deposit (= top of substrate)
track_height_layers = 5                                # number of deposit layers in y
y_track_max         = (ny_substrate + track_height_layers) * dy

# Initial conditions for a *newly born* cell (when laser arrives):
#   phase = 0  (liquid)
#   dT    = +1.0  (above melting in this dimensionless code)
#   P     = identity (liquid orientation, consistent with phi_liquid mask)
phase_birth = 0.0
dT_birth    = 1.0
...
# ============================================================
# Axis-angle sampling (Haar-uniform SO(3))
# ============================================================
def random_axis(rng):
    v = rng.normal(size=3)
...
# >>> NEW: laser y-position (vertical) is now at the TOP of the deposit track,
# mirroring the original where y_center sat at the original top surface ny*dy.
y_center  = y_track_max
z_center  = nz * dz / 2.0          # unchanged: middle of z (transverse)
x_center0 = nx * dx / 6.
...
# ============================================================
# Liquid (phase==0) orientation = identity
# ============================================================
phi_liquid = 1e-3
...
# ============================================================
# Background dT face conductivities (constant base; effective values are
# multiplied by the face-born mask each step so unborn cells are isolated).
# ============================================================
DT_base_xf = DT * jnp.ones((nx+1, ny, nz), dtype=jnp.float64)
DT_base_yf = DT * jnp.ones((nx, ny+1, nz), dtype=jnp.float64)
DT_base_zf = DT * jnp.ones((nx, ny, nz+1), dtype=jnp.float64)

bc_T_val = jnp.array(-3.0, dtype=jnp.float64)

# ============================================================
# Cubic symmetry (device)
# ============================================================
...
# ============================================================
# >>> NEW: cell-born -> face-born mask
# Interior face is "active" only if BOTH adjacent cells are born;
# boundary face is "active" only if the touching cell is born
# (this makes Dirichlet BC contributions vanish on unborn cells automatically).
# ============================================================
def born_to_face_mask(born):
    bx = jnp.zeros((nx+1, ny, nz), dtype=jnp.float64)
    bx = bx.at[1:nx].set(born[0:nx-1] * born[1:nx])
    bx = bx.at[0].set(born[0])
    bx = bx.at[nx].set(born[nx-1])

    by = jnp.zeros((nx, ny+1, nz), dtype=jnp.float64)
    by = by.at[:, 1:ny].set(born[:, 0:ny-1] * born[:, 1:ny])
    by = by.at[:, 0].set(born[:, 0])
    by = by.at[:, ny].set(born[:, ny-1])

    bz = jnp.zeros((nx, ny, nz+1), dtype=jnp.float64)
    bz = bz.at[:, :, 1:nz].set(born[:, :, 0:nz-1] * born[:, :, 1:nz])
    bz = bz.at[:, :, 0].set(born[:, :, 0])
    bz = bz.at[:, :, nz].set(born[:, :, nz-1])
    return bx, by, bz

# ============================================================
# One time step (now also takes & returns the `born` field)
# ============================================================
@jax.jit
def one_step(phase, dT, P9, Pfunc, t, x_center, born):
    cond1 = (t > 0.0) & (t < 0.500)
    cond2 = (t > 0.500) & (t <= 0.505)
    cond3 = (t > 0.505)   # laser on, deposition starts here

    q_scalar     = jnp.where(cond1, 100.0, 0.0)
    T_mod        = jnp.where(cond3, 1.0, 0.0)
    source_coeff = jnp.where(cond3, 1.0, 0.0)

    # cond2 quench applies ONLY to already-born (substrate) cells
    dT = jnp.where(cond2 & (born > 0.5), -3.0, dT)

    # Move laser
    x_center = x_center + jnp.where(cond3, laser_speed * dt, 0.0)

    # ============================================================
    # >>> NEW: BIRTH LOGIC
    # When the laser is on and within the domain, every deposit cell
    # whose (x,y,z) lies inside the track footprint AND whose x has
    # been reached by the laser becomes "born" with liquid+hot IC.
    #   - track is along x (laser path)
    #   - track is centered in z, half-width = track_half_width_z
    #   - track is in y in [y_track_min, y_track_max]
    # ============================================================
    laser_active   = cond3 & (x_center <= (nx * dx))
    in_track_z     = jnp.abs(Z3 - z_track_center) <= track_half_width_z
    in_track_y     = (Y3 >= y_track_min) & (Y3 <= y_track_max)
    laser_reached  = X3 <= x_center
    should_be_born = laser_active & in_track_z & in_track_y & laser_reached

    born_new  = jnp.maximum(born, should_be_born.astype(jnp.float64))
    just_born = (born_new > 0.5) & (born < 0.5)

    # IC for newly born: liquid + hot + identity orientation
    phase = jnp.where(just_born,         phase_birth, phase)
    dT    = jnp.where(just_born,         dT_birth,    dT)
    P9    = jnp.where(just_born[None, ...], ID_P9, P9)

    born = born_new

    # ============================================================
    # Face-born masks => isolate unborn cells from the linear systems
    # ============================================================
    bxf, byf, bzf = born_to_face_mask(born)

    # Source term: only deposit energy into cells that exist
    source_term = lax.cond(
        x_center <= (nx * dx),
        lambda _: gaussian_source_jax(X3, Y3, Z3, x_center, y_center, z_center, laser_power),
        lambda _: jnp.zeros((nx, ny, nz), dtype=jnp.float64),
        operand=None
    )
    source_term = source_term * born

    # cond1 quench applies only to already-born (substrate) cells
    q = q_scalar * born
    ...
    # Mask face misorientation across unborn cells
    Pg_x_faces = Pg_x_faces * bxf
    Pg_y_faces = Pg_y_faces * byf
    Pg_z_faces = Pg_z_faces * bzf
    ...
    # Defensive: keep unborn cells frozen at phase=0
    phase = jnp.where(born > 0.5, phase, 0.0)

    # ============================================================
    # Temperature solve
    # ============================================================
    DT_xf_dyn = DT_base_xf * bxf
    DT_yf_dyn = DT_base_yf * byf
    DT_zf_dyn = DT_base_zf * bzf

    bc_on_f = jnp.where(cond3, 1.0, 0.0).astype(jnp.float64)

    diag_dT_diff_no  = diag_diff_no_bc(DT_xf_dyn, DT_yf_dyn, DT_zf_dyn)
    diag_dT_diff_dir = diag_diff_dirichlet_faces(DT_xf_dyn, DT_yf_dyn, DT_zf_dyn)
    diag_dT_diff     = diag_dT_diff_no + bc_on_f * (diag_dT_diff_dir - diag_dT_diff_no)

    rhs_dT_bc_dir = rhs_dirichlet_faces_3d(DT_xf_dyn, DT_yf_dyn, DT_zf_dyn,
                                           bc_T_val, bc_T_val, bc_T_val, bc_T_val)
    ...
    def A_T_mv(x1d):
        u = unflattenF_jax(x1d)
        Au = apply_A_diff_dirichlet_faces_scaled(DT_xf_dyn, DT_yf_dyn, DT_zf_dyn, u, bc_on_f) \
             + ((1.0/dt) + q) * u
        return flattenF_jax(Au)
    ...
    # Defensive: keep unborn cells at dT=0
    dT = jnp.where(born > 0.5, dT, 0.0)
    ...
    # Mask orientation conductivities by face-born too
    Kx = Kx * bxf
    Ky = Ky * byf
    Kz = Kz * bzf
    ...
    # Defensive: unborn cells -> identity orientation
    P9 = jnp.where(born[None, ...] > 0.5, P9, ID_P9)

    t = t + dt
    return phase, dT, P9, Pfunc, t, x_center, born

# ============================================================
# Field initialization
#   Substrate: cells with j in [0, ny_substrate)
#   Deposit  : cells with j in [ny_substrate, ny)  (initially unborn)
# ============================================================
phase_np = np.zeros((nx, ny, nz), dtype=float)
dT_np    = np.zeros((nx, ny, nz), dtype=float)
# Substrate keeps its original IC of -0.5; deposit cells stay at 0 until born.
dT_np[:, :ny_substrate, :] = -0.5
...
# >>> NEW: birth field. Substrate is born at t=0; deposit cells start unborn.
born_np = np.zeros((nx, ny, nz), dtype=float)
born_np[:, :ny_substrate, :] = 1.0
...
# >>> NEW: only seed inside the substrate region in y
ymin, ymax = Rseed, ny_substrate*dy - Rseed
...
# ============================================================
# Seed initialization (only in the substrate)
# ============================================================
orientations = generate_orientations_cubic(rng, n=numSeeds, min_sep_deg=5.0)

max_place_tries = 20000
placed = 0

# safety mask: never seed above the substrate
in_substrate = np.zeros_like(phase_np, dtype=bool)
in_substrate[:, :ny_substrate, :] = True
...
for i in range(numSeeds):
    ...
    for _try in range(max_place_tries):
        ...
        empty = phase_np < 0.5
        seed_new = seed & empty & in_substrate
...
print(f"Placed {placed}/{numSeeds} seeds in substrate.")
...
born  = jnp.asarray(born_np,  dtype=jnp.float64)
...
x_center = jnp.array(x_center0, dtype=jnp.float64)

for step in range(1, steps + 1):
    phase, dT, P9, Pfunc, t, x_center, born = one_step(phase, dT, P9, Pfunc, t, x_center, born)
    ...
    if (step % plot_every) == 0 or step == steps:
        ...
        born_h  = np.array(jax.device_get(born))
        ...
        if 0.500 < float(t) < 0.510 or step == 8000:
            fields = {
                "phi":          phase_h,
                "dT":           dT_h,
                "angn_over_pi": angn_over_pi,
                "born":         born_h,            # >>> NEW: 1 = exists, 0 = not yet deposited
                "P11h": P11h, "P12h": P12h, "P13h": P13h,
                "P21h": P21h, "P22h": P22h, "P23h": P23h,
                "P31h": P31h, "P32h": P32h, "P33h": P33h,
            }
        else:
            fields = {
                "phi":          phase_h,
        ...
        print(f"Saved VTK at t = {float(t):.6f}, x_laser = {float(x_center):.4f}, "
              f"#born_deposit_cells = {int(np.sum(born_h[:, ny_substrate:, :] > 0.5))}, "
              f"-> {vtk_filename}")

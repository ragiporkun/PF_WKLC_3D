...
# ============================================================
# IPF “standard triangle” mapping + MRD contours (your IPF code)
# ============================================================
...
def main():
    ...
    for ax, hkl, title in zip(axs, hkls, titles):
        ...
        ax.set_title(f"Pole Figure {title} (cubic, full net)")
    fig_pf.tight_layout()
    save_figure(fig_pf, args.outdir, args.prefix, "pole_figures_fullnet", formats, args.dpi)
    ...
    ax.set_title("Stereogram: crystal [001] in sample (full net)")
    fig_st.tight_layout()
    ...
    fig_ipf.suptitle(
        f"IPF (cubic, MRD contours). RD={args.rd}, TD={args.td}, ND={args.nd} (build dir defaults to +Y)",
        y=0.98, fontsize=12
    )
    fig_ipf.tight_layout(rect=[0, 0, 1, 0.95])
    save_figure(fig_ipf, args.outdir, args.prefix, f"ipf_contours_RD{args.rd}_TD{args.td}_ND{args.nd}", formats, args.dpi)
    ...
    ax3.set_title("Euler cube scatter (cubic reduced ~[0,90]^3)")
    fig_eu.tight_layout()

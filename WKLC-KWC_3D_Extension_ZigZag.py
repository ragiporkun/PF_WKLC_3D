...
v_x = 10.
v_z = 5.                                        # <<< was v_y
y_center = ny * dy                           # <<< now FIXED (was dynamic y_center0)
z_center0 = nz * dz / 4                          # <<< initial z (was fixed z_center)
x_center0 = nx * dx / 8
z_min_laser = nz * dz / 4.0                      # <<< was y_min_laser
z_max_laser = 3.0 * nz * dz / 4.0                # <<< was y_max_laser
x_steps_per_seg = 20
...
@jax.jit
def one_step(phase, dT, P9, Pfunc, t, x_center, z_center,
             x_step_count, laser_phase, z_dir):
    """
    Zig-zag laser now moves in the x-z plane:
      Phase 0 = moving along x for x_steps_per_seg steps
      Phase 1 = moving along z towards the opposite z-limit
    y_center is a module-level constant.
    """
    cond1 = (t > 0.0) & (t < 0.500)
    ...
    # ============================================================
    # Zig-zag laser movement in x-z plane
    # Phase 0 = moving along x for x_steps_per_seg steps
    # Phase 1 = moving along z towards the opposite z-limit
    # ============================================================
    active = cond3 & (x_center <= (nx * dx))

    # --- Phase 0: x-movement ---
    moving_x = active & (laser_phase == 0)
    x_center = x_center + jnp.where(moving_x, v_x * dt, 0.0)
    x_step_count = x_step_count + jnp.where(moving_x, 1, 0)

    # Check if x-segment is complete
    x_seg_done = moving_x & (x_step_count >= x_steps_per_seg)
    laser_phase = jnp.where(x_seg_done, 1, laser_phase)
    x_step_count = jnp.where(x_seg_done, 0, x_step_count)

    # --- Phase 1: z-movement (was y-movement) ---
    moving_z = active & (laser_phase == 1) & (~x_seg_done)
    z_center = z_center + jnp.where(moving_z, z_dir * v_z * dt, 0.0)

    # Check if z reached its target limit
    z_up_done   = moving_z & (z_dir > 0) & (z_center >= z_max_laser)
    z_down_done = moving_z & (z_dir < 0) & (z_center <= z_min_laser)
    z_seg_done  = z_up_done | z_down_done

    # Clamp z to the limit and switch back to x-movement
    z_center    = jnp.where(z_up_done,   z_max_laser, z_center)
    z_center    = jnp.where(z_down_done, z_min_laser, z_center)
    laser_phase = jnp.where(z_seg_done,  0, laser_phase)
    z_dir       = jnp.where(z_up_done,  -1.0, z_dir)
    z_dir       = jnp.where(z_down_done,  1.0, z_dir)
    ...
    t = t + dt
    return phase, dT, P9, Pfunc, t, x_center, z_center, x_step_count, laser_phase, z_dir
...
z_center_dyn = jnp.array(z_center0, dtype=jnp.float64)   # <<< dynamic z (was y_center)
x_step_count = jnp.array(0, dtype=jnp.int32)
laser_phase = jnp.array(0, dtype=jnp.int32)     # 0 = x-movement, 1 = z-movement
z_dir = jnp.array(1.0, dtype=jnp.float64)        # +1 = towards z_max, -1 = towards z_min

for step in range(1, steps + 1):
    phase, dT, P9, Pfunc, t, x_center, z_center_dyn, x_step_count, laser_phase, z_dir = \
        one_step(phase, dT, P9, Pfunc, t, x_center, z_center_dyn,
                 x_step_count, laser_phase, z_dir)
    ...
    if (step % plot_every) == 0 or step == steps:
        ...
        if 0.500 < float(t) < 0.510 or step == 90000: #or step == 50000 or step == 75000:
            fields = {
                "phi": phase_h,
                "dT": dT_h,
        ...
        print(f"Saved VTK at t = {float(t):.6f}, laser=({float(x_center):.4f}, {float(z_center_dyn):.4f}) to {vtk_filename}")
